...
export default function AuditoriaInventarioPage() {
  ...
  // Helper para fechas locales estables
  ...
  // Configuración de estilos dinámicos según la pestaña activa
  const CONFIG = {
    faltantes: {
      color: "orange",
      bgBtn: "bg-orange-600",
      ...
      bgLightTab: "bg-orange-50/40",
      label: "FALTANTES.LOG",
      subLabel: "Auditoría de Productos",
      tablaSupabase: "prod_faltantes"
    },
    sobrantes: {
      color: "indigo",
      bgBtn: "bg-indigo-600",
      ...
      bgLightTab: "bg-indigo-50/40",
      label: "SOBRANTES.PROD",
      subLabel: "Control de Inventario",
      tablaSupabase: "prod_sobrante"
    }
  }[tab];
  ...
  async function fetchProductos() {
    setLoading(true);
    let query = supabase.from(CONFIG.tablaSupabase).select("*").order("id", { ascending: false });
  ...
  async function guardarRegistro() {
    ...
    const { error } = await supabase.from(CONFIG.tablaSupabase).insert([{
    ...
    <>
      ...
      {notificacion && (
        <div className="fixed top-6 right-6 z-[1000] animate-in slide-in-from-right duration-500">
          <div className={`bg-slate-900 text-white px-6 py-4 rounded-2xl shadow-2xl border-l-4 ${tab === "faltantes" ? "border-orange-500" : "border-indigo-500"} flex items-center gap-3`}>
            <div className={`p-1 rounded-full ${tab === "faltantes" ? "bg-orange-500/20 text-orange-500" : "bg-indigo-500/20 text-indigo-500"}`}>
      ...
      {mostrarConfirmacion && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
          ...
          <div className={`bg-white rounded-[3rem] p-10 shadow-2xl border-t-[15px] ${tab === "faltantes" ? "border-orange-600" : "border-indigo-600"} text-center relative z-10 animate-in zoom-in duration-200 max-w-sm w-full`}>
            <h2 className="text-4xl font-black text-slate-900 uppercase italic tracking-tighter mb-6 leading-none">REVISAR<br/>REGISTRO</h2>
            <div className="bg-slate-50 p-6 rounded-[2rem] border-2 border-slate-100 mb-8 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-4 border-t border-slate-200 pt-4">
                ...
                <div>
                  ...
                  <p className={`text-2xl font-black ${CONFIG.text}`}>S/ {(parseFloat(nuevo.cantidad) * parseFloat(nuevo.precio)).toFixed(2)}</p>
                </div>
              </div>
            </div>
            <div className="flex gap-4">
              ...
              <button ref={btnConfirmarRef} onClick={guardarRegistro} className={`flex-1 ${CONFIG.bgBtn} text-white font-black py-5 rounded-2xl shadow-xl uppercase text-[11px] border-b-4 ${CONFIG.borderBtn} active:border-b-0`}>CONFIRMAR</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-7xl mx-auto space-y-8 p-4 min-h-screen flex flex-col relative bg-slate-50">
        ...
        <div className="flex flex-col lg:flex-row justify-between gap-6 shrink-0 pt-4">
          <div className="lg:text-left">
            <p className={`text-[10px] font-black ${CONFIG.text} uppercase tracking-[0.4em] mb-1 ml-1`}>{CONFIG.subLabel}</p>
                    <h2 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter uppercase italic">
              {tab === "faltantes" ? "FALTANTES" : "SOBRANTE"}<span className={CONFIG.text}>{tab === "faltantes" ? ".LOG" : ".PROD"}</span>
            </h2>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 bg-white p-3 rounded-[2.5rem] shadow-xl border border-slate-100 w-full sm:w-auto">
            {/* SWITCH INTERCAMBIO DE PESTAÑAS */}
            <div className=" flex bg-slate-100 p-1 rounded-2xl w-full sm:w-auto">
              <button 
                onClick={() => tab !== "faltantes" && setTab("faltantes")} 
              ...
              <button 
                onClick={() => tab !== "sobrantes" && setTab("sobrantes")} 
            ...
            {/* SWITCH PERIODO FECHA */}
            <div className="flex items-center gap-3 px-4 cursor-pointer select-none" onClick={togglePeriodo}>
              <span className={`text-[10px] font-black uppercase transition-colors ${esSemana ? CONFIG.text : 'text-slate-300'}`}>Semana</span>
              <div className={`relative w-16 h-8 rounded-full transition-colors duration-300 ${esMes ? 'bg-slate-900' : CONFIG.bgBtn}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 flex-1 pb-10">
          ...
          <div className="lg:col-span-4 self-start sticky top-4">
            <div className={`bg-white p-8 rounded-[3rem] shadow-2xl border-b-[12px] ${CONFIG.borderCard}`}>
              <div className="space-y-5" onKeyDown={(e) => e.key === "Enter" && !mostrarConfirmacion && revisarRegistro()}>
                <div>
                  <label className={`text-[10px] font-black ${CONFIG.text} uppercase mb-2 block tracking-widest ml-1`}>
                    {tab === "faltantes" ? "Producto" : "Descripción"}
                  </label>
                  <input 
                    autoFocus 
                    placeholder={tab === "faltantes" ? "Producto..." : "EJ. POWERADE AZUL"} 
                    value={nuevo.producto} 
                    onChange={(e) => setNuevo({ ...nuevo, producto: e.target.value })} 
                    className={`w-full bg-slate-50 border-2 border-slate-100 p-4 rounded-2xl text-sm font-black text-slate-900 focus:bg-white transition-all outline-none uppercase ${tab === "faltantes" ? "focus:border-orange-600" : "focus:border-indigo-600"}`} 
                  />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className={`text-[10px] font-black ${CONFIG.text} uppercase mb-2 block tracking-widest ml-1`}>Cantidad</label>
                    <input type="number" placeholder="0" value={nuevo.cantidad} onChange={(e) => setNuevo({ ...nuevo, cantidad: e.target.value })} className={`w-full border-2 border-slate-100 bg-slate-50 p-4 rounded-2xl text-center text-2xl font-black text-slate-900 outline-none transition-all ${tab === "faltantes" ? "focus:border-orange-600" : "focus:border-indigo-600"}`} />
                  </div>
                  <div>
                    <label className={`text-[10px] font-black ${CONFIG.text} uppercase mb-2 block tracking-widest ml-1`}>Precio Unit.</label>
                    <input type="number" placeholder="0.00" value={nuevo.precio} onChange={(e) => setNuevo({ ...nuevo, precio: e.target.value })} className={`w-full border-2 border-slate-100 bg-slate-50 p-4 rounded-2xl text-center text-2xl font-black text-slate-900 outline-none transition-all ${tab === "faltantes" ? "focus:border-orange-600" : "focus:border-indigo-600"}`} />
                  </div>
                </div>
                <button onClick={revisarRegistro} className={`w-full ${CONFIG.bgBtn} text-white font-black py-6 rounded-2xl shadow-xl uppercase text-xs tracking-widest border-b-4 ${CONFIG.borderBtn} active:border-b-0 transition-colors`}>
                  REGISTRAR {tab === "faltantes" ? "FALTANTE" : "PRODUCTO"}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-8 bg-white rounded-[3.5rem] shadow-xl border border-slate-100 flex flex-col overflow-hidden min-h-[500px]">
            <div className={`bg-slate-900 p-6 flex flex-col sm:flex-row justify-between items-center gap-4 border-b ${tab === "faltantes" ? "border-orange-600" : "border-indigo-500"}`}>
              <div>
                 <p className={`text-[9px] font-black ${tab === "faltantes" ? "text-orange-400" : "text-indigo-400"} uppercase tracking-widest mb-1`}>
                  {tab === "faltantes" ? "Auditoría de Stock" : "Stock Excedente"}
                 </p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className={`px-5 py-3 rounded-2xl flex items-center gap-2 shadow-lg border-b-4 ${CONFIG.bgBtn} ${CONFIG.borderBtn}`}>
                  <span className={`text-[9px] font-black ${CONFIG.textLight} uppercase tracking-widest`}>
                    {tab === "faltantes" ? "Total:" : "Sobrante:"}
                  </span>
              ...
              <table className="w-full border-separate border-spacing-y-2">
                ...
                <tbody className="divide-y divide-slate-50">
                  {productos.map((item) => {
                    ...
                    return (
                      <tr key={item.id} className={`transition-colors group ${tab === "faltantes" ? "hover:bg-orange-50/40" : "hover:bg-indigo-50/40"}`}>
                        <td className="px-6 py-3 bg-slate-50 rounded-l-2xl">
                          <div className={`flex flex-col items-center justify-center w-10 h-10 rounded-xl border border-slate-200 ${esHoy ? `${CONFIG.bgBtn} ${tab === "faltantes" ? "border-orange-600" : "border-indigo-600"} shadow-md` : 'bg-white'}`}>
                            <span className={`text-sm font-black leading-none ${esHoy ? 'text-white' : 'text-slate-900'}`}>{dia}</span>
                            <span className={`text-[7px] font-black leading-none mt-0.5 ${esHoy ? CONFIG.textLight : CONFIG.text}`}>{mes}</span>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-3 bg-slate-50 text-center">
                          <span className={`px-4 py-1.5 rounded-xl font-black text-xs ${tab === "faltantes" ? "bg-orange-100 text-orange-700" : "bg-indigo-100 text-indigo-700"}`}>{item.cantidad}</span>
                        </td>
                        <td className="px-6 py-3 bg-slate-50 rounded-r-2xl text-right">
                          <p className={`text-sm font-black font-mono ${tab === "sobrantes" ? "text-indigo-600 text-lg" : "text-slate-900"}`}>S/ {(item.cantidad * item.precio).toFixed(2)}</p>
                        </td>
                      </tr>
                    );
                  })}
