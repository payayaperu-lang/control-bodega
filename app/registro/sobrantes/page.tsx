...
function SobrantesForm() {
  ...
  async function guardar() {
    ...
    if (!error) {
      ...
      // Resetear estado local para habilitar escritura manual
      setNuevo({ producto: "", cantidad: "", precio: "" });
      setProductoBloqueado(false);
      setPrecioBloqueado(false);
      setMostrarConfirmacion(false);
      
      // Limpiar los parámetros de la URL para añadir otro producto sin trabas
      router.replace("/registro/sobrantes");
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100">
        <div>
          <h2 className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.3em] mb-1">Control Excedentes</h2>
          <h1 className="text-3xl md:text-4xl font-black text-slate-900 italic uppercase tracking-tighter">SOBRANTES<span className="text-indigo-600">.PROD</span></h1>
        </div>

        {/* Barra Central con Botón Volver y Filtros de Fecha */}
        ...
        <div className="bg-indigo-600 w-full md:w-auto px-6 py-4 rounded-2xl shadow-xl border-b-4 border-indigo-800 text-white flex justify-between md:block items-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4">
          <div className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-xl border-b-[10px] border-indigo-600 space-y-4">
            
            {/* Input Nombre Producto */}
            <div>
              <div className="flex justify-between items-center mb-1">
                <label className="text-[9px] font-black text-slate-400 uppercase block">Nombre Producto</label>
              ...
              <input 
                ...
                className="w-full bg-slate-50 border-2 border-slate-100 p-4 rounded-xl font-black text-slate-900 outline-none focus:border-indigo-500 uppercase transition-all text-sm disabled:opacity-60 disabled:bg-slate-100 disabled:cursor-not-allowed disabled:border-slate-200" 
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border-2 border-slate-100 p-4 rounded-xl text-center font-black text-xl outline-none focus:border-indigo-500 transition-all" 
                />
              </div>
              <div>
                <div className="flex justify-between items-center mb-1">
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border-2 border-slate-100 p-4 rounded-xl text-center font-black text-xl outline-none focus:border-indigo-500 transition-all disabled:opacity-60 disabled:bg-slate-100 disabled:cursor-not-allowed disabled:border-slate-200" 
                />
              </div>
            </div>

            <button onClick={validarYRevisar} className="w-full bg-indigo-600 text-white font-black py-5 rounded-xl shadow-lg uppercase text-xs tracking-wider border-b-4 border-indigo-800 active:translate-y-0.5 active:border-b-0 transition-all pt-4">
              REGISTRAR SOBRANTE
            </button>
          </div>
        </div>
        ...
        <div className="lg:col-span-8 bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden flex flex-col min-h-[400px]">
          <div className="p-6 bg-slate-900 text-white flex justify-between items-center">
            <span className="text-xs font-black uppercase tracking-wider text-indigo-400">Registros Encontrados ({productos.length})</span>
          </div>
          ...
          <div className="p-4 flex-1">
              ...
              <table className="w-full border-separate border-spacing-y-2">
                ...
                <tbody>
                  {productos.map(item => {
                    ...
                    return (
                      <tr key={item.id} className="bg-slate-50 hover:bg-indigo-50/40 transition-colors">
                        <td className="px-6 py-3 rounded-l-xl">
                          <div className="flex flex-col items-center justify-center w-10 h-10 rounded-lg bg-white border border-slate-200 font-black">
                            ...
                            <span className="text-[7px] text-indigo-600">{mes}</span>
                          </div>
                        </td>
                        <td className="px-6 py-3 font-black text-slate-900 uppercase text-sm">{item.producto}</td>
                        <td className="px-6 py-3 text-center"><span className="px-3 py-1 bg-indigo-100 text-indigo-700 font-black text-xs rounded-md">{item.cantidad}</span></td>
            ...
            <div className="block md:hidden space-y-3">
              {productos.map(item => {
                const { dia, mes } = formatFechaCorta(item.fecha);
                return (
                  <div key={item.id} className="bg-slate-50 p-4 rounded-2xl border-l-4 border-indigo-500 flex justify-between items-center shadow-sm">
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-sm font-black font-mono text-slate-900">S/ {(item.whitespace * item.precio || item.cantidad * item.precio).toFixed(2)}</p>
                    </div>
                  </div>
                );
              })}
      ...
      {mostrarConfirmacion && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 backdrop-blur-md bg-slate-900/80 animate-in fade-in duration-200">
          <div className="bg-white rounded-[2.5rem] p-6 md:p-10 max-w-sm w-full text-center border-t-[12px] border-indigo-600 shadow-2xl">
            ...
            <div className="bg-slate-50 p-4 rounded-xl text-left mb-6 space-y-1">
              ...
              <p className="text-xs font-black text-indigo-600 uppercase"><span className="text-slate-400">Total:</span> S/ {(parseFloat(nuevo.cantidad) * parseFloat(nuevo.precio)).toFixed(2)}</p>
            </div>
            <div className="flex flex-col gap-2">
              <button onClick={guardar} className="w-full bg-indigo-600 text-white font-black py-4 rounded-xl uppercase text-xs shadow-lg tracking-wider">SÍ, GUARDAR</button>
              <button onClick={() => setMostrarConfirmacion(false)} className="w-full bg-slate-100 text-slate-400 font-black py-3 rounded-xl uppercase text-xs">VOLVER</button>
            </div>
          </div>
        </div>
      )}
